...
fn fb_params_tys(ps: Vec<ParamN>, i: i64, acc: Vec<Bind>, tps: Vec<String>) -> Vec<Bind> {
    ...
    let ty = tk_solid(mark_tparams(taint_strip((ps)[(i) as usize].ty.clone()), &tps), &acc);
...
fn fb_function(ts: Vec<Token>, i: i64, sp: FbSpan, gtys: Vec<Bind>) -> Verd {
    ...
    let v = fb_walk(steps.clone(), 0i64, fb_params_scope(sp.ps.clone(), 0i64, vec![]), fb_params_tys(sp.ps.clone(), 0i64, gtys.clone(), tps.clone()), tk_solid(mark_tparams(sp.ret.clone(), &tps), &gtys)).v;
    if (v.v != "") {
        return v;
    }
