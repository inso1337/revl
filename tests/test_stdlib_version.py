...
def test_stamp_is_the_expected_bumped_counter():
    ...
    # of shipping a stale stamp. It is "6" as of `stdlib/framing.rvl` (issue
    # #867); "5" was `stdlib/escape.rvl` (item 459, issue #722); "4" was
    # `stdlib/a2a.rvl` (item 439, issue #118); it was "3"
    # for the two additions that landed without a bump (fs.rvl
    # is_dir/lexists/resolve_within, shell.rvl plan_op_name). Raise it here in
    # lock-step with the next public change.
    assert EXPECTED_STDLIB_VERSION == "6"
    assert read_stamp(STDLIB) == "6"
