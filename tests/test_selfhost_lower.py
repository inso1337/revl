...
def test_the_type_layer_gap_is_exactly_23_fixtures():
    ...
    pin addresses its fixtures by bare name under `examples/rejections/`), and
    30 until the provide-method / component slice closed the whole
    `provide-method and component bodies` family, all seven of it."""
    assert len(_TYPE_LAYER_CASES) == 23, len(_TYPE_LAYER_CASES)
    names = [name for _, name, _ in _TYPE_LAYER_CASES]
    assert len(set(names)) == 23, "a fixture is listed twice"
