...
def test_the_type_layer_gap_is_exactly_16_fixtures():
    ...
    more, and the declared-type slice (T1) `t6_bare_generic`; on top of those,
    the name-RESOLUTION rule (docs/design/457, the G1 read position) took the
    last two of the G1/G6 family. The twelfth document that moved with T3a,
    `dynamic_reserved_key`, never had a row here because this pin addresses its
    fixtures by bare name under `examples/rejections/`."""
    assert len(_TYPE_LAYER_CASES) == 16, len(_TYPE_LAYER_CASES)
    names = [name for _, name, _ in _TYPE_LAYER_CASES]
    assert len(set(names)) == 16, "a fixture is listed twice"
