...
# --- the census / provenance / construct-reach coupling -------------------- #
def test_a_census_change_selects_the_construct_reach_oracle():
    """issue #1215's rule, which issue #1315 found had never run: the
    `gate_census` row of tools/oracle_construct_reach.py imports the census for
    its corpus walk and its fast engine, so a census change has to re-run it."""
    r = sel("tools/gate_reference_census.py")
    assert r["full"] is False
    for node in ("tests/test_oracle_construct_reach.py",
                 "tests/test_gate_reference_census.py",
                 "tests/test_corpus_provenance.py"):
        assert node in r["pytest"], (
            f"a census change does not select {node}; it lived in a second, "
            "unreachable arm for the whole of issue #1215's life"
        )


def test_a_provenance_change_keeps_the_coupling_without_the_oracle():
    """The other half of the merged arm. The census/provenance coupling holds
    both ways, but the construct-reach oracle reads the census only, so folding
    the two rules together must not widen what a provenance change runs."""
