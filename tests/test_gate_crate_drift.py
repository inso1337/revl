...
def test_the_native_emitter_is_generated_into_the_crate():
    """Roadmap item 146 condition 4: `selfhost/emit_rust.rvl` is IN the crate.

    Structural, so it runs in the frontend job on every PR where no cargo
    exists. The byte-agreement claim is `tests/test_gate_crate_admit.py
    ::test_emit_ir_is_byte_identical_to_the_reference`, which needs one.

    Three things are pinned, because each can regress on its own:

    * the emitter and its IR constructor are in the emitted module's `use`
      closure, and the closure is in the digest, so removing either reds the
      drift gate rather than quietly shrinking the crate;
    * the shim exposes the arm and calls the emitter through the module;
    * `serde_json` carries `preserve_order`. Without it a record's fields come
      back sorted instead of in document order and the emission diverges from
      the reference on every record-shaped document, while every byte-exact emit
      oracle stays green (they run the emitter under CPython, where dict order
      is insertion order). A silent, oracle-invisible divergence is exactly the
      kind a committed artifact has to pin.
    """
    meta = json.loads((CRATE / "GENERATED.json").read_text(encoding="utf-8"))
    assert meta["native_emitter"] == "selfhost/emit_rust.rvl"
    for rel in ("selfhost/emit_rust.rvl", "stdlib/json.rvl", "stdlib/value.rvl"):
        assert rel in meta["selfhost_closure"], (
            f"{rel} must be in the emitted module's closure")
        assert rel in meta["digest_inputs"], (
            f"{rel} decides generated bytes, so it must be a digest input")

    lib_rs = (CRATE / "src" / "lib.rs").read_text(encoding="utf-8")
    assert "pub fn emit_ir(ir_document: &str, tier: Tier) -> Result<String, Verdict>" in lib_rs
    assert "selfhost::emit_rust_from_ir(" in lib_rs, (
        "`emit_ir` must call the emitted emitter, not re-derive an emission")

    selfhost_rs = (CRATE / "src" / "selfhost.rs").read_text(encoding="utf-8")
    assert "pub fn emit_rust_src(" in selfhost_rs
    assert "pub fn emit_rust_from_ir(" in selfhost_rs

    cargo = (CRATE / "Cargo.toml").read_text(encoding="utf-8")
    assert 'serde_json = { version = "1", features = ["preserve_order"] }' in cargo, (
        "the emitted crate reads an erased Value through serde_json's map; "
        "without preserve_order a record's fields come back sorted and the "
        "emission diverges from the reference")


def test_the_emit_arm_issues_no_admission_either():
    """The native emitter is a new surface, and the crate's central property has
    to hold across it: no arm of `emit_ir` may read as an admission. An emission
    is not a verdict, and a caller reading the fixed `{admitted, code, message}`
    shape must still see `false` on every refusal it can receive."""
    lib_rs = (CRATE / "src" / "lib.rs").read_text(encoding="utf-8")
    code = "\n".join(line for line in lib_rs.splitlines()
                     if not line.lstrip().startswith(("//!", "///", "//")))
    emit_arm = code.split("pub fn emit_ir(", 1)[1].split("\npub fn ", 1)[0]
    assert "admitted: true" not in emit_arm and "admitted:true" not in emit_arm
    assert "Verdict::OutsideFrontier" in emit_arm and "Verdict::Refused" in emit_arm
