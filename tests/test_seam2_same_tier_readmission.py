...
def test_boot_wiring_wires_the_same_tier_sync_seam():
    ...
    running = compile_files([_APP])
    info = {"socket": "provider.sock", "methods": ["query", "execute"],
    ...
    ok, reason = _pr._boot_wiring_decision([_APP], running, info)
    assert ok is True and reason is None

    assert asyncio.run(_pr._apply_boot_wiring("db", info, [_APP], running, wire)) is True
