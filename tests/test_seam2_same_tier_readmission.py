...
def _seam_dir(tmp_path, spec=None, name="consumer"):
    """A realistic placement directory and the address anchor a process booted
    out of it holds (item 337), mirroring `tests/test_swap.py:_seam_dir`. The
    conductor makes one 0700 directory per placement, writes each process's spec
    into it, and binds every seam socket there, so a receiver sanctions a UDS
    address by containment in the directory its own argv named."""
...
def test_boot_wiring_wires_the_same_tier_sync_seam(tmp_path):
    ...
    The seam is otherwise entirely honest, which is the point: `PgDatabase` is
    the provider of `db` in this process's own manifest, and the socket lies in
    the placement directory it was handed its own spec in, so the key binding
    and the address sanction (item 337) both pass and the only remaining
    question is the admission one this test is about."""
    running = compile_files([_APP])
