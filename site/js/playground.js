...
function renderAudit(data) {
  ...
  for (const name of names) {
    ...
    if (emis.length) {
      ...
      bnd += ` <span style="color:var(--faint);font-size:12px">(${b.compensated || 0} of them compensated)</span>`;
    }
    ...
    if (hostComp.length) {
      bnd += ` <span style="color:var(--faint);font-size:12px">(host compensated: ${esc(hostComp.join(", "))})</span>`;
    }
    if (externs.length) {
