...
func stash_path(p string) RevlResult[Stash, FsError] {
	bak := p + ".bak"
...
func (revlSelf *Agent_ops) Touch(p string) {
	var _revlWit1 RevlResult[Stash, FsError]
	_revlWit1 = stash_path(p)
			...
			defer func() {
				if r := recover(); r != nil {
					revlSelf.revlFrame.recordResidue(RevlTeardownRecord{
						Kind: "restore-residue", CrossingKey: "stash_path", CrossingMethod: "unstash",
						AttemptedCall: "unstash", AttemptedPhase: 1,
						ErrorType: "panic", ErrorMessage: fmt.Sprint(r),
						Outcome: "failed", Referent: "stash_path",
