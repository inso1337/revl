...
type Groups interface {
	...
	Set(key string, tags []string)
}

var _keyG = stc.NewKey[Groups]("g")

func Tagger() stc.Component {
	return stc.Component{
		Name:    "Tagger",
		Provide: []stc.Key{_keyG},
		Apply: func(ctx *stc.Context) (stc.Inverse, error) {
			...
			_implG := &Tagger_g{ctx: ctx, store: store}
			if _, err := ctx.Provide(_keyG, Groups(_implG)); err != nil {
				return nil, err
			}
			_ = store
			return nil, nil
...
func LoadTagger(target *stc.Context) *stc.Fiber {
	return target.Load(Tagger())
}
