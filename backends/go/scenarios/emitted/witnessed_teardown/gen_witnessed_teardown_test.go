...
func C() stc.Component {
	return stc.Component{
		...
		Apply: func(ctx *stc.Context) (stc.Inverse, error) {
			...
			if err := ctx.Effect(func() stc.Inverse {
				lockOp()
				return func() error { unlockOp(); return nil }
			}); err != nil {
				return nil, err
			}
			...
			if err := ctx.Effect(func() stc.Inverse {
				...
				return func() error {
					if _revlFrame.committed {
						return nil // item 247 a5a: discharge — never runs
					}
					_revlFrame.enqueue("writeOp", "writeOp", func() error { writeOp("delete"); return nil })
					return nil
				}
			}); err != nil {
				return nil, err
...
func CAbort() stc.Component {
	return stc.Component{
		...
		Apply: func(ctx *stc.Context) (stc.Inverse, error) {
			...
			var _revlWit2 RevlResult[Stash, FsError]
			if err := ctx.Effect(func() stc.Inverse {
				_revlWit2 = stash()
				if _revlOk2, _revlIsOk2 := _revlWit2.(RevlOk[Stash, FsError]); _revlIsOk2 {
			...
			return nil, fmt.Errorf("deliberate abort (slice 2b proof)")
			return func() error { _revlFrame.commit(); return nil }, nil
		},
	}
}

func LoadC(target *stc.Context) *stc.Fiber {
	return target.Load(C())
}

func LoadCAbort(target *stc.Context) *stc.Fiber {
	return target.Load(CAbort())
}
...
func TestCommitPathBracketRevertsWitnessedPersistsCompensationDischarges(revlT *testing.T) {
	root := stc.New()
	_fibers := map[string]*stc.Fiber{}
	{
		_f := LoadC(root)
		if err := _f.Ready(stdctx.Background()); err != nil {
			revlT.Fatalf("%s: %v", "lifecycle test \"commit path: bracket reverts, witnessed persists, compensation discharges\": load C", err)
		}
		_fibers["C"] = _f
	}
	if _f, _ok := _fibers["C"]; _ok {
		_f.Dispose()
	...
	if !(len(root.Fibers()) == 0 && revlHostLive() == 0) {
		revlT.Fatalf("lifecycle test \"commit path: bracket reverts, witnessed persists, compensation discharges\": residue \u2014 %d fiber(s), %d host resource(s) (R4/R1)", len(root.Fibers()), revlHostLive())
	}
}
