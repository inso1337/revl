...
class _V3Emitter:
    ...
    def _helper_list_at(self) -> str:
        ...
        """
        return """  (func $list_at (param $list i32) (param $i i64) (result i32)
    ...
    def _index_expr(self, node: dict, scope: _Scope, where: str) -> _E:
        ...
        if _is_list_type(target_ty):
            ...
            target = self._expr(node.get("target"), scope, where, target_ty)
            # `$list_at` narrows the Int index to the i32 address space and
            ...
            address = f"(call $list_at {target.wat} {index.wat})"
...
def _refuse_deferred_emissions(ir: dict) -> None:
    ...
    tier's existing "never silent degradation" refusal channel. The reachability
    check and the single canonical wording live in `revl.session_commit`, shared
    by all five ownerless tiers so six backends do not invent six messages; a
