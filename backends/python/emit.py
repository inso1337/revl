...
class _ComponentEmitter:
    ...
    def _method(self, out: _Lines, indent: int, provide_name: str, method: dict, where: str) -> None:
        ...
        if params != list(spec.get("params") or []):
            raise EmitError(f"{where}: method {name!r} params do not match the service")
        mwhere = f"{where}.{name}"
...
def emit(ir: dict) -> str:
    ...
    out = _Lines()
    out.add(0, '"""Generated by the revl cordis-py backend (ir_version 0) — do not edit.')
    out.add(0)
    ...
    for service_name, service in services.items():
        ...
        for method_name, spec in (service.get("methods") or {}).items():
            out.add(
                2,
                f"{method_name!r}: {{'params': {list(spec.get('params') or [])!r}, "
