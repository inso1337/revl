...
def test_user_cache_emits_java_structure():
    ...
    assert 'fx.track(ctx.provide(ServiceKey.of(Cache.class, "cache")' in src
    assert "this.db.execute(" in src
    assert 'UnsupportedOperationException("effectful method body")' not in src


def test_requires_binds_the_provider_named_by_its_key():
    """Two providers of one service type, distinguished by provision key: a
    consumer that requires one key binds THAT provider.

    The service key an emitted `provide` registers, and the one an emitted
    `requires` resolves, both carry the provision key — so `ctx.get(..)` picks
    the provider the consumer named rather than whichever provider of the class
    happened to register last. This mirrors how the go/rust/python/wasm tiers
    already route (each keys a provision by name, not by service type).
    """
    ir = compile_source(
        """
        service Store {
        ...
        component ReadOnlyStore provides readonly: Store {
          let s = effect Map.new() undo s.drop()
          provide readonly {
            fn put(v) { effect s.insert("k", v) undo s.remove("k") }
          }
        }

        component AdminStore provides admin: Store {
          let s = effect Map.new() undo s.drop()
          provide admin {
            fn put(v) { effect s.insert("k", v) undo s.remove("k") }
          }
        ...
        component App requires readonly: Store {
          effect readonly.put("x") undo readonly.put("")
        }
        """
    )
    src = _emit(ir)
    # each provider registers under its own key, so both coexist
...
def test_v2_realms_emit_isolate_and_intercept():
    ...
    assert 'ctx.intercept(ServiceKey.of(Kv.class, "kv"), java.util.Map.of("quota", 5L, "tags", java.util.List.of("tenant_a")));' in src
    assert 'ctx.intercept(ServiceKey.of(Kv.class, "kv"), java.util.Map.of("quota", 9L, "tags", java.util.List.of("tenant_b")));' in src
...
def test_java_compensation_discharges_on_clean_unload_stub_runtime(tmp_path):
    ...
    runner.write_text(
        ...
        "    public static void main(String[] args) throws Exception {\n"
        "        Context ctx = new Context();\n"
        "        ctx.provide(ServiceKey.of(revl.Components.Bus.class, \"bus\"), new Rec());\n"
