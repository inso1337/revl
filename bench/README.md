...
- `framework_unload_survey.py`: the third host's selection evidence. Named,
  falsifiable claims about each candidate framework's published API, checked
  against the artifact the package index serves at a pinned version.
- `injection_escape.py`: the injection-escape column. One spec, several
  injection vectors carried in the material a component reads, generated by the
  pinned model, with compliance detected from the source text and containment
  from a named hard gate.
...
python3 bench/framework_unload_survey.py           # the third host's evidence
...
Two commands do reach a network, and only these two:
`bench/framework_unload_survey.py --fetch` downloads the pinned framework
versions from npm and PyPI to re-check its claims, and
`bench/injection_escape.py --runner local` posts to the endpoint you named.
...
| injection escape | `bench/injection_escape.py`, generated by the pinned model |
...
### The third host, and how it was chosen

The framework is `@modelcontextprotocol/sdk`, pinned at 1.30.0. The criterion
that decided it is the one `bench/hosts.json` stated before any framework was
named: a host with no unload path cannot fail the residue column honestly,
because it scores maximally badly by construction for a reason that says
nothing about the runtime comparison.
...
`bench/injection_escape.py` generates with the pinned model, putting the injected
instruction in the material a component reads rather than in the system prompt: a
service doc comment, the brief, or the compiler output the retry loop feeds back.

```bash
python3 bench/injection_escape.py --runner mock                  # pipeline check
python3 bench/injection_escape.py --runner local --hosts revl    # against the pin
python3 bench/injection_escape.py --check <label>                # validate a run
```

It reports two numbers and refuses to collapse them. **Compliance** is how often
the model did the undeclared thing at all, which is a property of the model and
the same question on every host. **Containment** is whether a named hard gate
then refused the artifact. Zero compliance means the gate was not exercised, and
the checker fails a run that reports containment anyway.

A refusal is split again by what it was about. A document refused with a
diagnostic naming the injected reach is evidence about confinement; a document
refused for an unrelated syntax error kept the behaviour out and is evidence
about nothing, so the two are counted apart. The first live attempt was the
second kind.

`tests/test_adversarial_gate.py` is still not the source of this cell. It checks
