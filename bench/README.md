...
## FRAMEWORK-BENCH-1: the named three-host comparison

Roadmap item 548 (issue #1267), designed in `docs/design/562-framework-benchmark.md`.
Same tasks, one pinned local model, three hosts: raw Cordis/TypeScript, an agent
framework, revl. The refusal column leads.

```bash
python3 bench/refusal_inventory.py --markdown      # the refused column alone
python3 bench/model_pin.py --samples 4 --write     # pin and probe the endpoint
python3 bench/framework_bench.py --check           # assemble + validate the report
python3 bench/framework_bench.py --write           # commit report.json + report.md
```

All of it is free except the pin probe, which talks only to an endpoint you
name on your own machine. Nothing here calls a third party.

**Run it against your own endpoint.** `bench/hosts.json` is the registry: it
names the hosts, the pinned model, the task set and the columns, so pointing the
suite at a different model or adding a framework does not mean editing the
harness.

```bash
python3 bench/model_pin.py --endpoint http://127.0.0.1:11434 \
    --model <your-tag> --kind ollama --samples 4 --write
```

A run against a different model is a valid run of this suite and an invalid
comparison against our table. The pin records which model produced every number
...
|---|---|
| **refused** | `bench/refusal_inventory.py`, recomputed from the ledgers the tests gate |
| admits | `bench/rescore.py` over a committed corpus, or a fresh `bench/run.py` |
| residue | `bench/score_raw_ts.py` via `tools/residue-probe` |
| injection escape | not implemented; see below |
| tokens to green | `bench/tokens.py` |
| admission latency | `bench/admission_latency.py` |

### Two asymmetries the report states rather than hides

**Raw TypeScript is scored on leaks, not compile-rate,** because TypeScript
always compiles. There is no first-pass compile number for that host: it would
read 100% on every attempt including the ones that leak on unload. One host is
scored on what it leaks and another on what it admits, and those are different
questions rather than two views of one.

**The refused column is the credibility of the table.** Every other column is
one revl is designed to win. If a run of this suite comes out with revl winning
every column, that is a finding about the table. The refusal counts are
recomputed at report time from `tests/test_selfhost_compile.py`,
`tests/fixtures/selfhost_blind_spots.json`,
`tests/fixtures/oracle_construct_reach_ledger.json` and
`tools/gate_reference_census_baseline.json`, including the fail-open direction,
where the embeddable gate admits programs the reference compiler refuses.

### What the committed report does not contain

`bench/results/framework-bench/report.md` lists its own remaining gates: no
agent framework is named yet, no cell was generated by the pinned model, the
injection-escape column has no runnable measure, every claim stands at the
`measured` rung because nobody outside this repository has reproduced it, and
nothing is published outside this repository.
