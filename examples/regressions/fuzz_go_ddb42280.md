...
- Found by: `tools/fuzz_cross_tier.py` (roadmap item 292 — the empirical counterpart to item 133), seed 2, program #3.
...
- Root-cause fingerprint: `assertion failed: revlEq(probe(), RevlErr[any, any]{Value: ""})`
...
--- FAIL: TestCrossTierProbe (0.00s)
    gen_test.go:28: assertion failed: revlEq(probe(), RevlErr[any, any]{Value: ""})
FAIL
FAIL	revltest	0.396s
FAIL
```

## Minimized program (`fuzz_go_ddb42280.rvl`)

```revl
pub fn probe() -> Result[Int, Str] { return Err("") }

test "cross_tier_probe" { assert probe() == Err("") }
```
