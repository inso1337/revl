...
def _calls(node) -> list[tuple[str, bool, list]]:
    ...
    statement calls, in evaluation order - INCLUDING the calls nested inside
    it.
    ...
    ORDER, the approval edge and the argument list, which a set cannot.
    ...
    postcondition to the TARGET a step acted on, and the target is an
    argument.
...
def _walk_calls(node, approved: bool,
                found: list[tuple[str, bool, list]]) -> None:
