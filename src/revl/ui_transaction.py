...
#: in order: the site-spelled inverse of an `emit ... compensate ...` (item 247)
#: and the `undo` half of an `effect`. Both are entries on the activation
#: frame's teardown accumulator, so they run on unwind rather than in the
#: forward order reported here, and whether a crossing HAS an inverse is
#: already carried per step by `_extern_index`. Walking them would report a
#: second crossing for every compensated one: the fail-CLOSED direction, but
#: still a wrong reading, because the plan would claim an actuation the program
#: never makes in that order.
...
def _calls(node) -> list[tuple[str, bool, list]]:
    ...
    statement calls, in evaluation order, INCLUDING the calls nested inside it.

    WHY THIS IS A FULL WALK AND NOT A LIST OF STATEMENT KINDS. It used to read
    exactly three shapes (`emit e`, `let x = e`, a bare expression statement)
    and only the TOP node of each one's expression. Five spellings of the same
    crossing therefore reached no plan at all on a tree that compiles today:
    `return emit click(t)`, the `then` arm and the `else` arm of an `if`,
    `n = emit click(t)`, and a call nested inside a larger expression
    (`emit click(t) + 0`). Each produced a plan with no step for the crossing,
    so it got no residue verdict, no confirmation state, no postcondition
    verdict, no `[4]` row in the erase report and no entry in
    `unconfirmedIrreversibleSteps`. That is the fail-open direction in the one
    namespace whose whole point is that an irreversible actuation is never
    silently uncovered, and tail position is where an actuation most naturally
    lands: a provide method that returns what it clicked has nothing left to
    bind (issue #1327).

    A list of statement kinds is what let that happen, and a longer list would
    only defer it: every step kind the language grows is another way to hide a
    crossing until someone remembers this table. So the walk is generic, it
    descends everything, and the two keys it does NOT descend are named with
    the reason in `_REGISTERED_INVERSE`. `emission_analysis._calls_in` makes
    the same call for the same reason; this one differs only in keeping ORDER,
    the approval edge and the arguments, none of which a set can carry.
    ...
    WHAT A BRANCH MEANS HERE. This is a READING, not an execution. Both arms of
    an `if` are reported, because either can run and the plan cannot know
    which. Reporting an arm a given run skips over-states the plan by one
    crossing; reporting neither under-states it by all of them, and only one of
    those two directions can leave an irreversible actuation unnamed.
    ...
    `with e`, the only form the parser admits it on, so each nested statement
    re-reads its own rather than inheriting an enclosing one.
...
def _walk_calls(node, approved: bool, found: list[tuple[str, bool, list]]) -> None:
    """`_calls`'s recursion. `approved` is the approval edge of the innermost
    enclosing statement; it is re-read at every statement boundary."""
    ...
    # `{kind: fn, name}`, a pure fn body to
    # `{kind: call, callee: {kind: var, name}}`. A method body is the first
    # today; an arrow inside one is the second, and a walk that knew only one
    # shape is half of the bug this replaces.
    name = None
    kind = node.get("kind")
    ...
    if name is not None:
        found.append((name, approved, list(node.get("args") or [])))
