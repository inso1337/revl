...
runnable in-process tier; ``rust``, ``java``, ``wasm`` and ``go`` are runnable
too, each as a separate process over the bridge seam (see :mod:`revl.run_rust`,
:mod:`revl.run_java`, :mod:`revl.run_wasm`, :mod:`revl.run_go` — --once boots
and tears down a real composition with a no-residue proof, behind a per-tier
runtime-availability gate). ``--backend ts`` is accepted but reports that it is
not wired yet (multi-runtime is a roadmap item, not a promise here).
"""
...
# py boots cordis-py in-process (the _Driver below); rust/java/wasm/go each boot
# the composition as a *separate process* over the same bridge seam (run_rust.py,
# run_java.py, run_wasm.py, run_go.py): rust a cordis-rs binary, java a JVM on
# cordis4j, wasm the cordis-wasm runtime on wasmtime, go the stc-go placement
# runner. On every non-py tier --once (boot -> LIFO teardown -> no-residue proof
# -> exit) is wired, each behind its own runtime-availability gate
# (skip-with-reason, never a green run that booted nothing).
RUNNABLE_BACKENDS = ("py", "rust", "java", "wasm", "go")
...
def run_command(args) -> int:
    ...
    if backend in ("rust", "java", "wasm", "go"):
        ...
        # different runtime): cordis-rs, cordis4j on a JVM, cordis-wasm on
        # wasmtime, or stc-go (docs/interop-bridge.md, docs/swap.md). --once is
        # wired on all four, each behind its own runtime-availability gate.
