...
def _boot_wiring_decision(spec_files: list, running_ir: dict, key: str, info: dict,
    ...
    proxy is never wired at all, and the caller makes that refusal fatal to the
    boot (`run()` -> `BootRefused`) rather than leaving the consumer up with an
    unwired dependency. Whether an entry is a same-composition seam at all is
    decided by the CALLER from the receiver's own manifest
    ...
    deferred seam, and carries no component/backend at all. This function still
    fails closed rather than assume-admit if such an entry reaches it without a
    selector.
    ...
    catch. Both run BEFORE the admission predicate: they are the cheap,
    receiver-derived half, and neither compiles anything.
...
async def run(spec: dict, spec_path=None) -> None:
    ...
    for key, info in (spec.get("proxies") or {}).items():
        ...
        # Which entries are in scope is read off OUR OWN manifest, never off the
        # entry's `remote` flag (`_providers_of`): a key some component of
        # `running_ir` provides is gated however the entry labels itself, and
        # only a key this composition provides nowhere is the genuine
        # cross-composition handoff of Seam 3 (item 151), which wires ungated
        # and so can never contribute to `refused`.
