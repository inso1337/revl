...
- **Conformance is measured, not asserted.** `tools/conformance.py` emits 59
  language constructs through all six backends and reports what each does (the
  generated matrix lives under [Conformance matrix](#conformance-matrix) below,
  regenerated by `make matrix` and gated against drift in CI);
...
| component/emit + compensate | ok | ok | ok | ok | ok | ok | lim |
