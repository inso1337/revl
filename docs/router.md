...
(rust/go/java/ts/wasm) is a follow-up — see §4. This note records the design,
why the routing lives in the driver rather than the emitted body, and what the
other tiers need.
...
* **Per-tier emitter routing (item 165's territory — `backends/*/emit.py`).**
  To make the *emitted* router body itself route (so `revl run --backend rust`
  and the other four tiers fan out without the py driver), each emitter must,
  for a key carrying a `routes` entry, emit **per-realm handle resolution**
  (the N handles, one per named realm) plus the selection + strict-liveness
  skip, rather than a single `inject`/committed-view read. Concretely, the
  emitter needs to lower a routed require into: (a) N realm-scoped resolutions
  of the key, and (b) a strategy driver over them that re-checks liveness per
  call. The py runtime shows the shape (`_Router` in `src/revl/run.py`); each
  tier reproduces it in its own runtime. This was **not** attempted here —
  `backends/*/emit.py` is another agent's exclusive territory.
