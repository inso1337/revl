...
94. 🚧 **`_tool_admit` compiles before honoring `replacing` (harness model
...
95. **Rust emitter: reuse-after-move at a service-call argument (harness
...
96. 🚧 **Lifecycle tests cannot advance the clock, so a timer's *firing* is
...
97. 🚧 **The harness's reader artifacts cannot cross the canonical ABI —
