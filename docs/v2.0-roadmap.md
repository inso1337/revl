...
104. ✅ **`Str.length` on a multibyte string literal — NOT A BUG (verified already-correct at 9510650).** Item 103's side-flag was a misattribution: the wasm literal `.length` already routes through `$str_cp_length` (code-point counter; Str became a code-point sequence in item 51). Wasmtime-proven: café=4, 日本語=3, naïve=5. Adding a regression-guard exec test for the literal path (had no direct pin).
