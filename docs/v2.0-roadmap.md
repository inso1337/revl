...
141. **ts dispatch-await gap — ts harness components need `await` at the dispatch site (harness build matrix, ts tier).** The ts tier builds 9/15; the tier-scoped blockers are `@py`-only externs (authoring — ts needs `@ts` bodies, not a bug) AND a real "dispatch-await gap" in the ts emitter (a dispatched call that should be awaited isn't). Investigate + fix the ts emitter so the harness's dispatch shape awaits correctly. `backends/typescript/emit.py`. Unblocks the ts tier's remaining harness components.
