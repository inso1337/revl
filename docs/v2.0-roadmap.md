...
10. **Instance-parametric components** — phase 1 landed, and ✅ **PHASE 2 DONE:**
    `spawn` lowering executes on **all six tiers** — cordis-py (reference),
    ts, rust, go, and java, each proven on its real runtime (the 4 spawn
    properties: distinct local realms, independent LIFO teardown, request-
    scoped anti-leak, supervision-tree addressing). Remaining: **wasm spawn**
    (now unblocked — rich types landed, see below), **hot-swap** with live
    instances, and an **instance accessor** — the phase-1 handle only exposes
    `.dispose()`, so reading a provision back from a spawned instance's realm
    isn't yet expressible in revl (a uniform limitation noted by every tier).
