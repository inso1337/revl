...
10. **Instance-parametric components** — phase 1 landed, and ✅ **PHASE 2 DONE
    ...
    **B3** (per-instance local/identity realms), a contained change in the
    ...
    and the cordis-wasm B3 commit is done **but not yet pushed to the
    cordis-wasm repo** — until it is, the wasm-spawn scenario skips on CI (runs
    locally). ✅ **Instance accessor** — reading a provision back through a
    handle — is landed on the frontend + cordis-py (`s.<key>` field access, no
    new grammar; frozen `instance-get` IR node in docs/design-v2-instances.md);
    **remaining: lower it on the other five tiers (fan-out).** Also remaining:
    **hot-swap** with live instances (migrate-state; in progress). Subsumes the
    old "dynamic realms" item.
