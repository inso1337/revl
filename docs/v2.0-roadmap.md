...
171. 🚧 **Path C slice 5a — arrow-color leak trio + `Async[T]` IR modeling (from item 169).** The last A1 surface: `_refuse_leaky_arrow` (provide-method sync-typed arrow), `_refuse_leaky_pure_arrow` (module-fn twin), and the module-fn first-class async value. Soundly mirroring them needs the gate to model which arrow argument lands in an `Async[T]` parameter slot (`_coerce_async_args`) — param-type coercion `selfhost/lower.rvl`'s IR doesn't carry yet; without it the gate would over/under-refuse vs the reference's admitted twin. Extend the gate IR with the Async[T] coercion, then mirror the trio. Completes the NON-spawn A1 surface. Also: route all in-file gate `test` assertions through the differential oracle (`_agree`) rather than hand-asserting (169's oracle-coverage lesson). `selfhost/lower.rvl` + `tests/test_selfhost_lower.py`.
