...
- [ ] **Inverse-descriptor spike** (gates 47): pick the three effect
...
- [ ] **Tokens-to-green baseline** (gates 50's tricks): one bench run
  with token counting, no optimizations — the number the tricks must
  beat.
- [ ] **Search-as-admission prototype** (gates 49's protocol): run the
  ...
  written.
- [ ] **Non-ASCII cross-tier probe** (gates 51's scope): one probe
...
10. **Instance-parametric components** — phase 1 landed (frozen `spawn` IR +
    cordis-py reference executing; docs/design-v2-instances.md, "Phase 1 —
    frozen"). Remaining: `spawn` lowering on the other four tiers, hot-swap
    with live instances. Subsumes the old "dynamic realms" item.
...
49. **The agent-first component registry — search is admission, trust
    is a dossier. UNBLOCKED: phased, and phase 0 starts now — full
    design spec in docs/registry.md (layout, `revl_resolve`
    request/response, ranking, reuse-hint rules, security stance, exit
    tests).** The §5
    stance said *not yet* on the collision argument — two independent
    authors both providing `db` collide without namespacing. Re-read
    against the agent-first use case, that argument gates the *publish
    side at scale*, not *reuse*: the core loop is search by structural
    need → fetch one candidate → admit into my composition, and if an
    agent somehow installs two providers of one key, **G2 already
    refuses it at admission** — the gate was always the collision
    protection. The demand is here in a form the original entry did
    not anticipate: the second author is an **agent**, agents
    re-create what they cannot discover, and every re-creation costs
    thousands of output tokens plus gauntlet cycles where an import
    costs two calls. So: phases, first-class for agents throughout.

    - **Phase 0 — the read path (no namespacing needed, buildable
      today).** A git-backed index — the registry *is* a repo (the
      early-Go-modules/nixpkgs model): components as source + manifest
      (28) + dossier, single namespace, first-come names, fine at
      current scale. One MCP verb, `revl_resolve`, takes the need — a
      service shape, or a hole's fill spec (32) verbatim — and returns
      ranked candidates **with source and manifest inline**, so
      resolve → admit is two round-trips, never a browse session. The
      §5 compatibility check is the search predicate; the de-risk
      probe above ("search-as-admission prototype") is this phase's
      feasibility gate and its first commit.
    - **Phase 0.5 — reuse as a diagnostic, not a policy.** At
      admission of a freshly generated component, run the structural
      match against the index; when a compatible published component
      exists, the result carries a non-blocking hint: "a published
      `UserCache` is §5-compatible with what you just wrote —
      `revl_resolve` first". The compiler already does *did-you-mean*
      for names; this is did-you-mean for **existence**. Agents follow
      diagnostics — make reuse the path of least resistance instead
      of an instruction they must remember.
    - **Phase 1 — the publish path.** `revl_publish` is one verb,
      gauntlet (31) pass required, manifest + dossier attached, still
      git-backed. Trust is attached evidence, not stars — and
      install-time re-verification is cheap because the gate is the
      install tool.
    - **Phase 2 — scale hygiene, exactly where §5 put it.** Key
      namespacing + versioning (item 9), update flows where a breaking
      reshape is detected by the same drift machinery that guards
      hot-swap, a hosted index — *when the second human author
      arrives*. The original demand test survives; it gates phase 2,
      not the registry.

    **Measured, not assumed:** reuse rate joins the bench — of the
    specs where a compatible component exists in the index, how often
    does the agent import vs regenerate, and at what token cost each
    (feeds 50's tokens-to-green). If agents don't reuse even when
    nudged, phase 1 waits and the entry says why.
...
69. **Record update and match-arm blocks — measured ergonomics.**
    Dispatched as run_00018, never landed. `{r | f = x}` (functional
    record update) and block-bodied match arms carry a measured cost
    receipt: the shadow2 probe burned **six refusals** on var-in-record
    ceremony that a record-update form removes outright. Spec-first as
    dispatched — design the two forms, implement only what stays
    small, and re-run the probe scenario as the acceptance test: the
    same task should compile with zero of those six refusals. (Map
    iteration/size/remove — run_00017, also unlanded — is *not* a new
    item: it is item 52's implementation, and 52's order-semantics
    decision gates it; the run's spec-first framing already agrees.)
