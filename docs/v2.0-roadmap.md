...
180. 🚧 **stdlib `Value`/`Json` type (or total `Any` ops) to navigate an IR document in PURE revl — the fundamental Path B enabler (slice-1 dogfood).** The checker refuses stdlib methods on an `Any` receiver (`node.args.length()` → "stdlib method `length` on a value of unknown type (G8)"), so a self-hosted emitter walking a dynamic `kind`-discriminated IR must either type the ENTIRE IR as an ADT or bridge every access through `@py` accessors (slice 1 did the latter). Every IR-consuming self-host stage (each Path B emitter) re-derives that bridge. Fix (design): a stdlib `Json`/`Value` type with total typed getters (`.str/.int/.list/.opt/.field`), OR a fixed set of TOTAL `Any` operations (get-field/length/index) erasing to host dynamic access. Makes pure-revl self-hosted backends tractable. HIGH — unblocks Path B cleanliness. `stdlib/` + `src/revl/typecheck.py`.
