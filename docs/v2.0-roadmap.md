...
extends Wave 3's persistence), 49 (registry — after item 9's
namespacing; its query/trust schema is being defined by 28/31/32
whether anyone notices or not), 41/42/43/45 as the ecosystem ring.
...
iteration ships, whenever that is).
...
- **A component repository before key namespacing** — the §5 "not yet"
  stands; item 49 is sequenced behind item 9's namespacing, not around
  it.
