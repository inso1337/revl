...
117. **A1/coloring is blind to handle async ops in arrow bodies (low priority; found in item 106).** `lower.py::_req_op_is_async`/`_arrow_reaches_async` only see `req`-target async ops, not `emit <handle>.<key>.<method>()`. So an UNCOLORED arrow reaching a handle async op (e.g. passed to a sync param) is neither refused by A1 nor colored async — a soundness gap the item-106 py-emitter fix + defensive `_revl_as_async` cover in the common (colored) case but not this one. Also docs/design/async-function-values.md:420 still shows the old `_revl_as_async` shape. Frontend fix in `lower.py`.
