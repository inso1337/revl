...
108. 🚧 **Item 137 landed (Branded->Str + literal-union->variant) but the
    ...
    With this, `PluginInventoryGateway.list(): PluginInventorySnapshot`
    (the DSH checkout's `packages/host/plugin-inventory/src/`) imports
    end to end.## Toward early production
