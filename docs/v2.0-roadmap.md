...
10. **Instance-parametric components** — phase 1 landed (frozen `spawn` IR +
    ...
    frozen"). 🚧 **PHASE 2 IN PROGRESS — claimed by backend-wave, do not pick
    up:** `spawn` lowering on ts/rust/java/go (each mirrors the py reference,
    executed on its real runtime); wasm spawn deferred until wasm rich types
    land (see the wasm-to-zero item). Remaining after: hot-swap with live
    instances. Subsumes the old "dynamic realms" item.
