...
75. **Errata harvest, checker side — the typing frontier's next
    ...
    the spelling should be TypeScript's, per §0.
    (b) **The stdlib-named-method sliver** — a receiver whose provenance
    no constructor pins refuses non-stdlib method names but *lowers a
    stdlib-named method as that builtin*, wrong at runtime: `x.get(k)`
    on an extern-returned value dispatches as the Map builtin. Refuse
    stdlib-named methods on unpinned receivers too (same HOST-METHOD
    diagnostic), so the fence closes to exactly "host-object results are
    on the audit surface" — the G8 line as designed, with no accidental
    hole through the builtin table. Same fault line, tooling half: the
    mapiter run proved the host/stdlib name collision is *latent* on the
    contributor side too — adding `remove` to `_BUILTIN_METHODS`
    silently reclassified every host stub's `store.remove(k)` and broke
    16 tests, with the disjointness invariant living only in a test
    assertion (findings-mapiter §2). Add the guard at the source: a
    checked assertion that the builtin and host-verb tables stay
    disjoint except where documented, failing at *table-edit* time with
    the collision named, not at golden-diff time.
    (c) **Type-parameter hygiene** — the implicit one-letter heuristic
    cannot be turned off, so a typo'd one-letter type name silently
    quantifies instead of erroring. When a fn carries an explicit `[T]`
    list, the heuristic should be *off* for that signature: declared
    means declared, and a stray one-letter name becomes the undeclared-
    type error it always should have been. Bounds (`[T: Ord]`) stay
    deferred — no consumer demands them yet; note the decision, don't
    build the machinery.
    Each closure follows the t8–t20 pattern: a rejection file naming the
    tier error it prevents, plus the false-positive test that keeps the
    legal spellings compiling. Exit: the errata entries for (a) and (b)
