...
41. 🚧 **The Component Model bridge — slices 1-2 ✅; slice-3 value-canonical ABI ✅ (Str+scalars+records+lists+variants lift/lower over cabi_realloc, wasmtime component-model proven, a39b02a); service-level (provide-method) canonical lowering is the remaining gap
...
96. **wasm  emits invalid wasm for a function whose whole body is a diverging  statement** (missing trailing ) — found during 41-s3 aggregates; affects the NORMAL wasm tier, not just the canonical path. . Bounded fix.

97. ** emits referenced / types at the WIT package top level** (invalid WIT — must be inside an ); found during 41-s3 aggregates, worked around locally with a relocate helper. . Bounded fix.
