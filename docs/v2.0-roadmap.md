...
95. 🚧 **Str and record lowering on the wasm tier (unblocks 41 slice-3 -> 45).** The wasm tier is i32-only; `Str`/records are hard EmitErrors, which is why 41 slice-3 (standard WASI-P2 canonical-ABI) and 45 (quarantine tier) are gated. Give the wasm emitter a linear-memory ABI: `Str` as (ptr,len), records as struct layout with field offsets, pinned by wasmtime execution tests. Str first, records second. Frontend already handles both; this is purely the wasm emitter.
