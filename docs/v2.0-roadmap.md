...
104. **`Str.length` on a multibyte string LITERAL returns the UTF-8 byte count, not code points** (`"café".length`->5) on the wasm tier — the literal fast-path constant-folds to `(i32.load)` instead of `$str_cp_length`. Contradicts docs/strings.md code-point-correctness; runtime-built Str values are unaffected. Found during item 103, confirmed on pristine main. `backends/wasm/emit.py`. Bounded fix.
