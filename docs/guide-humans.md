...
| `revl slo` | measure a recorded run against the composition's declared `slo { ... }` contract, take the declared `on breach` response, and refuse the next rollout when the generation it replaces breached an objective it still declares | [commands-reference.md](commands-reference.md#revl-slo) |
