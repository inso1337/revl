...
- [gate-dependency-contract.md](gate-dependency-contract.md) — the security contract for a host that depends on the gate as a library: `pip install revl` + `revl.gate` (the promised `__all__` surface), and the native `revl-gate` crate, whose verdict surface issues no admissions and whose separate admission surface is deliberately tiny
