...
peer-death, TCP+mTLS); the *stdlib components* that package it — the `Router`
and the multi-realm `require` syntax — are **planned** (roadmap items 161 and
162), not built. This document exists so that no future contributor, reaching
for the obvious-looking shortcut, quietly breaks the guarantee that makes revl
worth using.
...
against the repo; the two that are packaging-not-yet-done are marked **planned**.
...
| **Router lifecycle verification** | the router is a component; its own acquire/route/dispose is checked by **G7** (LIFO-complete derived teardown) and **A8** (mid-body acquire-failure reverts accumulated effects, lands FAILED, siblings unaffected) | **Built** (the guarantees; the *Router that uses them* is item 161) | DESIGN.md §4 (G7); docs/backend-ir-v1.md §A8 |
| **stdlib `Router` / `RoundRobin`** | the packaged component: `spawn_pool(n, factory)` with `undo dispose_pool`, provides the key by routing | **Planned — item 161** (~3–5 days; primitives exist, this packages them) | v2.0-roadmap.md item 161 |
| **Multi-realm `require` syntax** | `requires db in realms("w1","w2","w3") strategy(round_robin)` — bind all N realms at once | **Planned — item 162** (in progress) | v2.0-roadmap.md item 162 |

The honest summary: the *model* is fully expressible from shipped primitives;
the *ergonomics* (a one-line `requires db via RoundRobin[Database](3, PgDatabase)`)
are items 161/162. Nothing on the planned list needs new runtime semantics — 162
...
| **Load balancing** | router + spawned pool (one provider per worker realm; router provides the key in the parent realm) | Pattern buildable; stdlib `Router` = item 161 |
