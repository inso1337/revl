...
**Slice 1 - the smallest landable core (py tier, `PUT`-with-`GET`-preimage
witnessed write).** The minimum that proves the move end-to-end on one tier:

1. `import openapi`: the `x-revl-witnessed` / `x-revl-preimage` / `x-revl-undo`
   annotations and their `--witnessed`/`--preimage`/`--undo` engineer
   equivalents, honored **only on `PUT`** (verb gate, attack 3), promoting to the
   item-243 `witnessed[net.*]` extern with a network cap scope; refusal on
   `POST`/`PATCH`; the claim-vs-proof header block (§2/§3), including the
   `un-observes` sentence verbatim.
2. The generated `witnessed[net.<host>] fn ... -> Result[HttpWitness, HttpError]
   undo restore(result)` shape, reusing item 243's landed fourth classification,
   Ok-conditional auto-registration, and WAL-serializable witness unchanged - the
   witness carries URL + captured preimage bytes + reversal method/headers.
3. The **network cap scope** on the IR node, and the item-250 invariant
   (`witnessed[net.*] ⟹ enumerated-not-run by fork rewind`) pinned as a test  - 
   even though 250 itself is not in this slice, the scope discriminator it keys
   on ships here so 250 cannot regress it later.
4. py-tier host-body stub + one fixture endpoint round-tripped through
   `compile_files` (as `test_import_openapi.py` already does), plus the
   abort-LIFO test analog for two `PUT`s to one resource (§5/G7).

Deferred, explicitly:

- **`DELETE` witnessed-remove** (attack 2's soft-delete residue) - Slice 2.
- **`POST`-`compensate`-grade** - belongs to item 247's surface, wired here as a
  Slice 2 follow-up, not conflated with witnessed.
...
- **`If-Match`/`ETag` concurrency tokens** (attack 1) - Slice 2; Slice 1 states
  the TOCTOU limitation in the header rather than closing it.
...
- **item 248 measurement extension** to the network boundary - rides item 248,
  reads the witnessed/emission fractions this slice produces.

Slice 1 is additive: no existing generated program uses `x-revl-witnessed`, so
