...
// (and, per the Revision above, DELIBERATELY so: this witnessed[net.*] shape is
//  refused by the landed rule-3 check, which is why the feature is recast to
//  compensate-grade. Kept as a sketch of the superseded design, not a program.)
