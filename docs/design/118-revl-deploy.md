...
> **SUPERSEDED by Revision R1/R3.** This assumed a per-host sliced bundle across
> trust domains. The revised Slice 1 is a SINGLE trust domain that stages the
> WHOLE bundle and verifies the whole-composition signature on HMAC; the
> asymmetric upgrade moves to the cross-trust-domain future item, where the
> per-slice verify also requires a new segmented attestation primitive (R1).
...
> **SUPERSEDED by Revision R3.** Because the revised Slice 1 is a single trust
> domain, Ed25519 is DROPPED from Slice 1 (which ships on HMAC with "signer
> untrusted" documented as not-yet-enforced) and re-scoped as the asymmetric-
> signing migration (item 127 v2) prerequisite of the cross-trust-domain future
> item. See Revision R3 and the re-sliced plan.
