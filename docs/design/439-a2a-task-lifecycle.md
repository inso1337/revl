...
| G8 | the boundary surface is the four (or one) synthesized externs, enumerable on the boundary surface `revl audit` renders, each carrying the folded `net.<host>` reach. True because a remote row synthesizes an ORDINARY provider holding ORDINARY externs, and pinned over the composition's compiled document for both forms by `tests/test_439_a2a_transport.py::test_the_a2a_crossing_is_on_the_g8_audit_surface` and `::test_all_four_task_crossings_are_on_the_g8_audit_surface`. The `revl audit` CLI reads the same document: a composition argument is RESOLVED rather than compiled as a module, and the shapes it cannot resolve refuse by name instead of rendering the empty surface it used to (slice G8c, `docs/design/439-a2a-transport-binding.md`'s scope limits) |
