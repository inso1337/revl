...
behind a typed boundary, item 459) · **Status:** FINDINGS, 2026-09-08
...
- `examples/app/notes.rvl` grows the ambient `service WebUI` and a
  `component NotesConsole requires webui: WebUI` that, at activation, emits
  `webui.add_entry("./frontend/entry.client.ts", "./frontend/dist/.vite/manifest.json", ["/notes"])`.
...
- `examples/app/frontend/` is a normal Vite/Vue project (Vue 3 + Vite 7 +
  ...
  `NotesConsole.vue` is the screen, `contract.ts` is the shared typed channel, and
...
### G3 — the webui coeffect carries no typed reactive-state / RPC `data`, so the console channel contract is authored on the TS side, not projected from the service declaration
...
The landed webui coeffect cannot express it. `service WebUI`'s only emission is
`add_entry(dev_source, prod_manifest, routes) -> Str`; there is no `data`
parameter, and `examples/webui-entry/webui_host.ts` publishes Cordis'
`addEntry(files, {})` with an empty reactive surface deliberately (design note
530 Decision B, "the slice publishes an empty `data` surface deliberately"). So
the console's `NotesConsoleState`/`NotesConsoleRpc` contract
(`examples/app/frontend/contract.ts`) is authored on the TS side rather than
PROJECTED from the `Ranker` service declaration in `notes.rvl`, and the server
does not populate it.

This is **not a workaround absorbed by the app**: the console drives ALL note
state through the typed REST routes (the landed typed contract, 457 artifact 5),
which needs no `data` channel; `contract.ts` is written against the shape the
projection will emit so no client rewrite is needed when it lands. Filed against
**item 459 / 457**: design note 530 Decision B folds the typed reactive-state/RPC
projection into item 457 as `revl export client --lang ts --face webui` (457
slice S4). Closing it makes `contract.ts` a generated artifact (like
`notes.client.ts`) and lets `add_entry` take a typed `data` provision, at which
point the server's published fields and the browser's `useRpc` type are one
declaration and the reactive channel is typed end to end. Until then the reactive
half of the boundary is the filed gap; the REST half is real and typed today.
...
- `revl export client --face webui` (457 S4), which closes G3 and makes
  `contract.ts` a projected artifact.
