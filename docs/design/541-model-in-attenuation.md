...
**Item 544 landed first, written against this record's SHAPE.**
`src/revl/kernel_boundary.py::effective_from_model_reach` reads
`manifest["model_reach"]` and was merged while this slice was still a branch,
returning `[]` on every composition until the section arrives. So the three
things below are no longer a promise to a future consumer: they are the
contract a shipped one already reads, and the shape is fixed by it rather than
by this note. What #1223 consumes is exactly three things, and nothing more is
promised:
...
and is already on main (`lower._check_kernel_boundary`,
`kernel_boundary.effective_from_model_reach`). It was written against the
record shape in section 3.3 and has been inert since it merged, because no
composition carried a `model_reach` section for it to read. This note's S1 is
what supplies one. Section 3.3 says what it consumes.

**S5. The council members' reach.** A `model council` places several roles
under one arm (item 516), and the fold reads every MEMBER role, because a
council asks all of them. That much is here: `model_route.check` records a
council placement's `candidates` as its member roles, so a member reaching
past the component that routes to the council is refused with both sets named.
What is NOT here is a reach declared on the council rather than on its
members, and the aggregation's own reach - the council's answer is a join of
