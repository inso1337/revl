...
| a row in the tier matrix | none; the property was invisible to `tools/tier_guarantees.py` | one row, with an `ACKNOWLEDGED` reason and an expiry condition |
...
The cost 543 named is real and this note pays it in section 6: one
`ACKNOWLEDGED` entry, with the condition that removes it written down.
...
## 6. The tier matrix, and why the reproducers are not fixture files
...
A fixture there is not available yet. `examples/rejections/` is a census corpus
root (`tools/gate_reference_census.py` `CORPUS_DIRS`, walked with `rglob`) and
`selfhost/parser.rvl` answers any program that declares a `model council` with
`BAD|unexpected token at top level`, so a council fixture in that directory
enters the census as a divergence against a self-host that cannot read it. The
named self-host marker is a separate lane (issue #1291) and the port is item
516's slice 5. This is item 512's decision, held for the same reason and
recorded in the same place.

So the entry reads:

> issue #1190 registers the code with its reproducers as INLINE strings in
> `tests/test_council_disagreement_1190.py` and as `revl reject
> G-COUNCIL-SPLIT` fences in `docs/design/557-council-disagreement.md`,
> compiled by `tests/test_doc_examples.py`. ... Remove this entry when the
> self-host port lands (issue #1291); a stale acknowledgement fails this gate.

The expiry condition is the point. `tools/tier_guarantees.py` fails generation
in BOTH directions, so the entry cannot outlive the reason for it: once a
fixture under `examples/rejections/` is refused under this code, the
acknowledgement is stale and the gate says so.
...
| `tools/tier_guarantees.py` | one `ACKNOWLEDGED` entry, with its expiry condition |
