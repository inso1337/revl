...
The two amendments pull in opposite directions, one refuses more and one
refuses less, and they do not overlap. Relaxing step 4 to the in-place question
does not relax steps 2 and 3: a legitimate same-tier sync seam still has to be
bound to its key and carry an address inside this process's own placement
directory. Conversely a refusal from step 2 or 3 is exactly as fatal as one
from step 4, because the consequence is identical, an unwired dependency, and
`run()` does not distinguish them.
