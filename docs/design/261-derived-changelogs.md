...
**The completeness guard.** `derive_changelog` asserts, at build time, that
every top-level key in `audit_report(after)` is either (a) consumed by a
declared differ, or (b) named in an explicit `UNDIFFED_AUDIT_SURFACES` set whose
presence-with-a-delta emits an unclassified honesty line. A newly-added audit
surface (a future item's new key) therefore either gets a differ or trips the
guard's test - it cannot silently escape the changelog. This guard is the
mechanical enforcement of "no verifiable change is dropped."

The `--json` document shape:

```json
{
  "headline": {"bump": "major", "previousVersion": "1.4.2", "nextVersion": "2.0.0"},
  ...
  "unclassified": [{"fact": "audit-surface:recovery_surface", "text": "recovery surface changed - not yet differenced (2 entries moved); review manually"}],
...
(1) the COMPLETENESS GUARD - `derive_changelog` asserts every top-level
`audit_report` key is either differenced or in `UNDIFFED_AUDIT_SURFACES`, and a
key in that set that shows a raw delta emits an UNCLASSIFIED honesty line
("recovery surface changed - not yet differenced; review manually"). So Slice 1
is honest-but-incomplete: it never claims silence it cannot back, and it points
the operator at the surface. (2) Slice 2 adds `diff_recovery`, `diff_registers`,
`diff_cardinality`, `diff_capability_scopes` to `audit_diff.py` (their correct
home, reused by the gate too), moving each from unclassified to breaking. A
DROPPED recovery inverse and a WEAKENED register are then first-class breaking
lines with `lede=True`. **Status: mitigated by the guard in Slice 1, fully
closed in Slice 2. The guard is a Slice-1 exit gate, not optional.**
...
a scope that shrinks is added/relaxed. Until Slice 2, this delta falls under the
completeness guard's honesty line (the `boundary.capabilities` map is part of the
audit surface the guard watches). **Status: OPEN in Slice 1 (surfaced as
unclassified via the guard), closed in Slice 2.**
...
- `src/revl/changelog.py`:
  ...
  - `_completeness_guard(after_ir)` - asserts every `audit_report` top-level key
    is differenced or in `UNDIFFED_AUDIT_SURFACES`; emits unclassified honesty
    lines for undiffed surfaces that show a raw delta.
  - `derive_changelog(before, after, previous_version=None, no_semver=False) ->
    ...
    guard; returns the section-4 JSON shape; computes `headline = max(version,
    authority, wiring)`.
...
- Tests: fact-bijection (every line has a resolvable fact), determinism
  (byte-identical under re-render and re-serialized inputs), the CRITICAL
  regression (a dropped recovery inverse trips the guard's honesty line, not
  silence), degraded-input (bare audit doc -> structural changelog, headline
  withheld and stated), headline-never-understates (a body breaking line forces
  a >= major headline).
...
**Slice 2 - close the audit-surface gap (the CRITICAL and HIGH).**

- `src/revl/audit_diff.py`: `diff_recovery(prev, new)`, `diff_registers`,
  `diff_cardinality`, `diff_capability_scopes` - sorted added/weakened/tightened
  buckets, mirroring `diff_crossings`/`diff_reach`. These belong in `audit_diff`
  (the gate reuses them too), not in `changelog`.
- `changelog.classify` folds them in; each moves from unclassified to its class
  (dropped inverse / weakened register / raised ceiling / widened scope ->
  breaking `lede=True`). Shrink `UNDIFFED_AUDIT_SURFACES` accordingly.
- Tests: each new differ's soundness, and the CRITICAL/HIGH regressions flip from
  "surfaced as unclassified" to "classified breaking."
