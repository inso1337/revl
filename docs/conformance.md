...
- **Regenerating a golden plus reviewing its diff is always an acceptable
  ...
  `python3 backends/typescript/scripts/regen-golden.py` (and siblings — any
  tier's golden is regenerated by emitting the reference IR and writing the
  file), a reviewed diff, and a commit that carries both. A stale golden
  *noticed by a red test* is the same decision, made at the test.
