...
[--markdown]` · 62 cases × 6 emitters. The default sweep needs no toolchain
(every emitter is pure Python); `--validate` additionally hands each tier's
output to that tier's real compiler; `--execute` **runs** the cases that have an
answer and checks that every tier computes the same one; `--markdown` prints the
matrix as a table (plus a revl self-host column and a live emit-timing readout).
