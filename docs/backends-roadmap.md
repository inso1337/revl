...
| cordis-wasm | **user wrote it** (first-party) | 1 test file, wasmtime exec | i64 ✅; rich-type boundary ✅ (**19→7 gaps**, 7 genuine); **`spawn` ✅ lowered + executed** via B3 (per-instance realms in the cordis-wasm runtime) — B3 commit not yet pushed to the cordis-wasm repo |
...
- **B4 · spawn lowering across tiers** — ✅ **DONE on all six tiers** (py/ts/rust/go/java/**wasm**), each executed on its real runtime (all 4 spawn properties). wasm needed B3 (per-instance realms) in the cordis-wasm runtime — committed, pending a push to the cordis-wasm repo. **Instance accessor** (`s.<key>` provision read): ✅ frontend + cordis-py landed; **fan-out to the other 5 tiers remaining.** **Hot-swap** with live instances (migrate-state): in progress.
