...
> Every tier does **spawn**, the **instance accessor** (`s.<key>`), and **hot-swap** (migrate-state); **wasm-to-zero** landed (19→7 genuine boundaries); **cordis-go** is a full sixth tier at 0 conformance gaps. The only remaining backend item is **C / C++ / Zig**, which is *not* revl-side work — it is blocked on those Cordis runtimes existing (runtime-authoring, the user's domain, like cordis-wasm). revl's side starts the moment one runs.
...
- **B4 · spawn lowering across tiers** — ✅ **DONE on all six tiers** (py/ts/rust/go/java/**wasm**), each executed on its real runtime (all 4 spawn properties). wasm needed B3 (per-instance realms) in the cordis-wasm runtime — pushed to the cordis-wasm repo (`f5c0562`). **Hot-swap** with live instances (migrate-state): ✅ **landed** (`ffb41a7`; structural migration is the canonical substrate — item 53; the same-typed-reorder hazard is named + pinned). **Instance accessor** (`s.<key>` provision read): ✅ **DONE on all six tiers** — the fan-out merged (ts/rust/java/go/wasm each resolve through the spawn handle's stored instance context/realm), including the async-activation fix (`0c09da7`: spawn waits for the child to activate, so the read resolves through the live context).
