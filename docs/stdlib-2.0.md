...
Anything else is a compile error. There is no verbatim pass-through to whatever
...
| `checked_div_trunc(b)` | 1 | Int | total `div_trunc`: `Result[Int, Str]`, with `Err(reason)` at a zero divisor instead of a fault (docs/arithmetic.md) | built | built |
...
- `split` is pinned to the JS shape on every backend: `"a,,b".split(",")`
  ...
  regex-split drops it, so its lowering passes `Pattern.quote(sep), -1`), and
  `"abc".split("")` is the 1-char strings (Python's `str.split("")` raises, so
...
(`query`, `insert`, `drop`, ...). The checker tracks host provenance, so a
`let` bound to a host-constructor call infers the *family*, and it checks a
...
the call form. The stub's *result* stays opaque: no entry claims to know what
comes back. So values flowing out of host objects remain on the audit
...
safe because dispatch is by receiver kind: the table versus
...
canonical-ABI string/list model, and now also the reader trio, which closes the
harness's reader gap: `split` (`$str_split` → `List[Str]`), `join`
(`$str_join`), and `Str.indexOf` (`$str_index_of`, code-point index out).
...
that is the moment the tiers would otherwise diverge for free.
...
is Job's). In
particular the reader is `lookup`, not `get`, because `get` already means the
host stub's unchecked read and reusing the spelling would make `x.get(k)`
...
- **Equality** is the language's one structural equality (syntax-2.0
  ...
  the same key set and equal values under every key, so `{a=1, b=2} ==
...
- **Iteration order is sorted, not insertion.** When iteration ships,
  ...
  `Str` order** on every tier: Unicode scalar value, equivalently UTF-8 byte
  lexicographic. Order is a pure function of the key set,
...
- **Bottom-typed receivers learn `V` from use.** A bottom `V` is not a
  ...
  return another `Map[Str, Never]`, which flows into *any* `Map[Str, X]`,
  planting any value under any
...
and it needs no change to the representation table below, so the two costly
...
because revl's loop story is under design. Iteration ships as `keys(): List[Str]`
...
and a pure function of content. That is the same property map equality already
has.

**Removal is persistent and total.** `remove(k)` returns a NEW map without
`k`, exactly as `set` returns a new map with it, never a mutation, matching
...
on the v1 stub). That is the ONE sanctioned overlap between the two namespaces,
...
**total**, and that includes `Int.MIN`, whose magnitude has no positive
...
and a byte-comparison WAT helper on wasm, since a code-point prefix of a valid
...
- `is_alpha()`: one ASCII letter `a`–`z` or `A`–`Z`. **Letters only, and `_` is
...
code-point range compare. Lowered native as a chained comparison (`"0" <= x <=
...
outside `selfhost/lexer.rvl` uses these builtins yet, and that file is only
ever emitted through the python backend, so no other backend is exercised with
them.
...
`Int`, **directly**, with no intermediate 1-char `Str`. It is the codepoint-domain
...
small, because CPython caches 1-char Latin-1 strings, so the `charAt`
"allocation" was already near-free and only the fn call is reclaimed. On the
**native tiers** `charAt`'s lowering allocates a heap `String` (`…to_string()`
on rust) per byte,
...
itself, `"-9223372036854775808"`, parses, though its magnitude has no positive
...
"Sized integers"). The checker dispatches by receiver head, so `Int32.to_int`
widens and `Str.to_int` parses; a receiver that is neither (`Bool.to_int`,
...
`ct_equal` verifies an inbound webhook without a timing side channel (never
...
`random_token` mints a session id; the `emission` classification is visible in
the declaration:
