...
**cordis-py: an `await` in the body defeats A8's "lands FAILED". — CLOSED**

*Closed by the cordis-py fork (commit `1316174`, branch
`harden-fiber-lifecycle` — the vintage `backends/python/setup.sh` pins): the
fiber now records an async effect-setup failure on the owning fiber and
re-derives its state, so an `await`-carrying body lands `FAILED` exactly like
a synchronous one. The closure is locked by
`tests/test_fault_tests.py::test_an_await_body_lands_failed_since_the_cordis_py_fix`;
the divergence hint in `src/revl/fault.py::_judge` stays, as a diagnostic for
anyone running an older runtime vintage. The original report is kept below
for the record.*
