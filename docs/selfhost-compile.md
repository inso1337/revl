...
the native emitter **is** byte-exact on the reference IR. A one-sided claim would
not locate the defect: an emitter that diverged everywhere would also fail to
...
- `tests/test_selfhost_compile.py`: the fully-native byte-exact proof on all six
  ...
  (`JAVA_LOWER_GAP_DOCS`), the gate-admits-the-surface and refusal-composes
