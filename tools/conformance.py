...
# The guarantee x tier support matrix (issue #1197, roadmap item 523). The two
# blocks above answer "does every CONSTRUCT survive every tier?"; this one
# answers "what is GUARANTEED on the tier I ship?", which an adopter could
# previously only reconstruct by reading the marker tooling. It is generated by
# `tools/tier_guarantees.py` from the checker's own registers and spliced here,
# under the same walk and the same staleness gate as the other two, so none of
# the three can be refreshed while another rots.
...
def guarantee_block() -> str:
    """The guarantee x tier block, from `tools/tier_guarantees.py`.

    Imported rather than restated: the rows are `revl.diagnostics.GUARANTEES`
    and the cells come from the divergence registers and the marker checks, so
    there is nothing here to keep in step by hand.
    """
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    import tier_guarantees  # noqa: PLC0415 — resolved next to this file

    return tier_guarantees.block()
...
def tier_guarantees_markers() -> tuple[str, str]:
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    import tier_guarantees  # noqa: PLC0415 — resolved next to this file

    return tier_guarantees.START, tier_guarantees.END
...
def _write_readme(*, check_only: bool) -> int:
    ...
    updated = _splice_marked(updated, tier_guarantees_markers()[0],
                             tier_guarantees_markers()[1], guarantee_block())

    if check_only:
        if updated == text:
            print("docs/conformance.md matrix, emit sweep and guarantee x "
                  "tier matrix are up to date.")
            return 0
        print("docs/conformance.md matrix, emit sweep or guarantee x tier "
              "matrix is STALE — regenerate it with `python3 "
              "tools/conformance.py --write-readme` (or `make matrix`) and "
              "commit the result.", file=sys.stderr)
        return 1

    if updated == text:
        print("docs/conformance.md matrix, emit sweep and guarantee x tier "
...
def main() -> int:
    ...
    parser.add_argument("--write-readme", action="store_true",
                        help="regenerate the three generated blocks in "
                             "docs/conformance.md in place: CONFORMANCE-MATRIX, "
                             "CONFORMANCE-SWEEP and GUARANTEE-TIER-MATRIX")
    parser.add_argument("--check-readme", action="store_true",
                        help="exit non-zero if any committed block in "
                             "docs/conformance.md (the construct matrix, the "
                             "per-tier emit sweep or the guarantee x tier "
                             "matrix) differs from a fresh generation — the CI "
                             "staleness gate")
