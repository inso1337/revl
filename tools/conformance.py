...
def _markdown(report: dict, selfhost: dict[str, str] | None) -> str:
    ...
    out.append("")
    out.append(f"`ok` the emitter produces code · `lim` a deliberate tier limit "
               f"· **GAP** a real gap (emitter has no case). "
               f"`wasm` is revl's first-party runtime; `revl` is the self-host "
               f"pipeline compiling itself, where `ok` means byte-identical to "
               f"the reference emitter and `lim` is the self-host frontier.")
    out.append("")
