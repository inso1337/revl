...
def adapter_construct_reach() -> list[Task]:
...
def adapter_census_bypass() -> list[Task]:
    baseline = json.loads(
    ...
    for bucket, cases in sorted(baseline.get("buckets", {}).items()):
        head = bucket.split("/")[0]
        if head == census.HARD:
            impls = (REFERENCE, SELFHOST)
            engine = "selfhost/lower.rvl (the self-host admission engine)"
        elif head == census.ADMISSION:
            # `false-admission` is the native crate issuing an admission the
            # reference refuses: three engines, and NEVER_BASELINED, so a
            # member here is a live defect rather than a recorded one.
            impls = (REFERENCE, SELFHOST, NATIVE)
            engine = "crates/revl-gate (the native admission surface)"
        else:
            continue
        signal = Signal(impls=impls, breadth=breadth_of(corpus))
...
def adapter_unexplained_refusal() -> list[Task]:
...
def adapter_formal_obligation() -> list[Task]:
    explained = set(_dict_keys(ROOT / "src/revl/diagnostics.py", "GUARANTEES"))
...
def check(tasks) -> list[str]:
    ...
    Three ways to fail, and each of them can fire:

      * a rung with no task. The review is explicit that a tier which cannot be
        populated from the tree is a RED and not a lowered expectation, so
        closing every task on a rung is an event that must be noticed: the
        curriculum then needs a new source, not a smaller ladder.
      * a task naming an artifact that is not in the tree, which is the failure
        ...
        supposed to exclude.
      * two tasks with the same id, which would let one source's population
    ...
    for name in TIERS:
        if counts[name] == 0:
            problems.append(f"tier {name!r} has no task at HEAD "
                            "(a tier that cannot be populated is a RED)")
    seen: set[str] = set()
    for task in tasks:
...
def _report(tasks) -> str:
    counts = populations(tasks)
    lines = [f"{len(tasks)} derived tasks at HEAD"]
    for name in TIERS:
        lines.append(f"  {name:<7} {counts[name]:>4}")
    lines.append("")
