...
# SMALLER region, the one where the covered layer plus what the certifier types
# ITSELF is the WHOLE question.
#
# Two kinds of source are in it. An INTERFACE declaration — service method
# signatures and scalar type aliases — carries no term at all, so `admit_src`
# raising no objection to it is not a partial answer but the complete one. A
# COMPONENT whose provide-method bodies are parameter reads and calls on a
# service it requires carries terms, and the certifier types every one of them:
# each body is walked against the declared signature it implements, each call
# against the declared signature it reaches, over the same closed scalar
# vocabulary, with no arm that guesses a type and no arm that skips a token
# (docs/design/457 T6). What the reference decides about such a source, this
# decides.
#
# Measured, not assumed: the `false-admission` bucket of
# `tools/gate_reference_census.py` holds every issued admission against the
# reference over the whole corpus and the fuzz draw, and may never be non-empty.
ADMITTED_LAYER = ("interface declarations, and components whose provide-method "
                  "bodies are parameter reads and calls on a required service, "
                  "over a closed scalar type vocabulary; every term in the "
                  "region is one this gate types itself")
