...
* `selfhost/lower.rvl` and its `use` closure (`lexer.rvl`, `parser.rvl`),
  ...
  emitter half is not in the crate. The two tiers are blocked differently:
  `selfhost/emit_py.rvl`'s helper externs (`py_repr`, `mangle`, ...) are
  `@py`-only and have no rust form at all, while `selfhost/emit_rust.rvl` DOES
  build as rust since roadmap item 146 but is not generated into the crate, and
  its entry takes the interchange IR as an erased `cordis::Value` with no
  rust-side constructor. That is Stage 4's lane, and the crate says so, per
  tier, in its own `compile_to`.
...
# The self-host EMITTER carried in the same module (roadmap item 146 condition
# 4). `selfhost/emit_rust.rvl` is the one emitter that builds AND runs as rust:
# all six of its externs carry `@rs` bodies (condition 1) and item 146 closed the
# `Any`-erasure boxing that stopped it building (condition 2, #1121). Its entry
# takes the interchange IR as an `Any`, which erases to the opaque
# `cordis::Value`; `stdlib/json.rvl::json_parse` is the constructor for that slot
# (condition 3, #1143), so IR-JSON text in and rust source out is a plain
# `Str -> Str` call the shim can make.
SELFHOST_EMITTER = "selfhost/emit_rust.rvl"
SELFHOST_IR_CONSTRUCTOR = "stdlib/json.rvl"
...
DIGEST_INPUTS = (
    *SELFHOST_CLOSURE,
    ...
    # item 521: the reserved computer-use roots are a frontier table, derived
    # from this module the way the keyword table is derived from the lexer. It
    # is a digest input for the same reason those are: a root added here moves
    # the covered surface, and a `frontier_id` that did not move would claim
    # two gates cover the same surface when they do not.
    "src/revl/ui_family.py",
...
#: The self-host function a later slice will add when it ports item 521's
#: admission rules. It does not exist today, and that absence is the whole
#: point of the table below: while it is absent the self-host gate checks NO
#: reserved capability root, so every root is a frontier gap and a program
#: carrying one is DECLINED BY NAME rather than met with silence (design
#: `docs/design/532-typed-computer-use.md` §9, which calls agreement by
#: silence the thing the marker exists to prevent).
#:
#: Unlike the keyword and builtin anchors above, a MISSING function here is
#: legal rather than fatal. The two cases differ in which direction the
#: absence moves the table: a missing `fn keywords()` would empty the
#: reference side and silently shrink the excluded set, while a missing
#: function here leaves the self-host side empty and WIDENS it, which is the
#: direction this crate is allowed to err in.
SELFHOST_UI_FN = "fn reserved_capability_root("


def _selfhost_capability_roots() -> set[str]:
    """The reserved capability roots `selfhost/lower.rvl` demonstrably checks.

    Empty until the port lands, at which point the roots the ported function
    names leave the excluded table in the same wave and the drift gate makes
    that visible.
    """
    src = (ROOT / "selfhost/lower.rvl").read_text(encoding="utf-8")
    start = src.find(SELFHOST_UI_FN)
    if start < 0:
        return set()
    end = src.find("\n}", start)
    body = src[start:end if end > 0 else len(src)]
    return set(re.findall(r't == "([^"\\]*)"', body))


def frontier_tables() -> dict[str, list[str]]:
    """`{"keywords": [...], "builtins": [...], "capability_roots": [...]}` —
    the reference constructs the self-host gate does not cover. Sorted, so the
    generated bytes are stable."""
    from revl.lexer import KEYWORDS  # noqa: PLC0415
    from revl.typecheck import _BUILTIN_SIG  # noqa: PLC0415
    from revl.ui_family import ROOTS as UI_ROOTS  # noqa: PLC0415

    roots = set(UI_ROOTS)
    # Anchored the way the two extractions above are, and in the direction
    # that matters: a silently-empty reference side would empty the excluded
    # table, which is the one direction this crate may not drift in.
    if not roots or not roots <= {r for r in roots if r and "." not in r}:
        raise SystemExit(
            f"build_gate_crate: the reserved capability roots look broken "
            f"({sorted(roots)!r}); the frontier table would be wrong, "
    ...
    return {
        ...
        "capability_roots": sorted(roots - _selfhost_capability_roots()),
    }
...
# `preserve_order` is LOAD-BEARING, not a preference. The native emitter reads
# the interchange IR through stdlib/value.rvl's `@rs` accessors, which downcast
# to a `serde_json::Value` and walk a record's fields in the order the map
# yields them. serde_json's default map is a `BTreeMap`, i.e. SORTED, and the
# reference emitter walks a Python dict, i.e. INSERTION-ordered — so without
# this feature `emit_ir` emits a record's members in the wrong order and
# diverges from the reference on every record-shaped document. Measured on the
# emit_rust corpus: 10 of 34 documents differ (records, variants, services,
# config). Every byte-exact emit oracle stays green either way, because those
# run the emitter under CPython.
...
TESTS_EMIT_RS = r'''//! The native EMITTER arm's own integration tests (`cargo test`), run with
...
def render_generated_json(digest: str, fid: str, language: str,
    ...
    payload = {
        ...
        # gate and called behind `stdlib/json.rvl::json_parse`, the constructor
        # for the `cordis::Value` its `Any` parameter erases to.
        "native_emitter": SELFHOST_EMITTER,
        "native_emitter_tiers": ["rust"],
        "emit_arm": (
            "revl_gate::emit_ir(ir_document, Tier::Rust) -> Result<String, "
            "Verdict>. A staged interchange IR document as JSON TEXT in, rust "
            "source out, with no Python and no reference compiler. Byte-agrees "
            "with backends/rust/emit.py over that backend's own self-host "
            "oracle corpus (tests/fixtures/emit_rust_corpus, 34 documents, no "
            "exclusions). It is not an admission and never reads "
            "`admitted:true`. `Tier::Py` fails closed: selfhost/emit_py.rvl's "
            "helper externs are @py-only. The FRONTEND is deliberately not in "
            "this path — `compile_to`, which would need one, still fails "
            "closed, because the self-host's lower_to_ir is behind the "
            "reference's (roadmap item 391)."),
        ...
        "admission_arm": (
            ...
            "verdict verbatim. Against a non-empty manifest a candidate is "
            "admitted when nothing it declares redeclares a service the running "
            "composition declares, read off the item-186 wire's service block "
            "(`!services` + `:S` rows); a wire with no block makes no claim, so "
            "any declared service is withheld, and a wire carrying a withdrawal "
            "row is declined outright."
        ),
        "manifest_arm": (
...
def render(tables: dict[str, list[str]], digest: str, fid: str, language: str,
           ...
           admission: dict[str, list[str]],
           emit_probe: dict[str, str]) -> dict[str, str]:
    ...
    return {
        ...
        "src/frontier.rs": (FRONTIER_RS_TEMPLATE
                            ...
                            .replace("@EXCLUDED_CAPABILITY_ROOTS@", _rust_str_array(
                                "EXCLUDED_CAPABILITY_ROOTS", tables["capability_roots"],
        ...
        "tests/emit.rs": (TESTS_EMIT_RS
                          .replace("@EMIT_PROBE_SOURCE@", emit_probe["source"])
                          .replace("@EMIT_PROBE_IR@", emit_probe["ir"])
                          .replace("@EMIT_PROBE_WANT@", emit_probe["want"])),
        "tests/ir.rs": TESTS_IR_RS,
        "tests/symbols.rs": TESTS_SYMBOLS_RS.replace("@FRONTIER_PROBE@", probe),
    }


# The generated module's ROOT DOCUMENT (roadmap item 146 condition 4).
#
# The crate carries two self-host halves that have to end up in ONE rust module,
# because the shim `mod selfhost;` is one file: the gate (`lower.rvl`) and the
# emitter (`emit_rust.rvl`). A revl document that `use`s both is how they are
# joined — the reference frontend merges the closure and the reference rust
# backend emits it, exactly as it does for any composition. No hand-written rust
# participates.
#
# It also spells the one function the shim cannot spell itself. The emitter's
# parameter is an `Any`, which erases to the opaque `cordis::Value`, and its
# constructor `stdlib/json.rvl::json_parse` is a `pub extern`, which the backend
# emits as a private `fn` — so `selfhost::json_parse` is not nameable from
# `lib.rs`. `emit_rust_from_ir` composes the two INSIDE the module, leaving the
# shim an ordinary `Str -> Str` call.
#
# The `use` paths are absolute so the document can be written into a scratch
# directory instead of into the repo tree. That leaks nothing into the emitted
# bytes — no path of the root document reaches the module — so generation stays
# deterministic across temporary directories (`test_generation_is_deterministic`,
# and `--check`, which regenerates into a fresh temp dir every time).
NATIVE_ROOT_TEMPLATE = """\
// GENERATED by tools/build_gate_crate.py -- the gate crate's emission root.
//
// `admit_src` / `admit_ambient` are the native gate; `emit_rust_from_ir` is the
// native rust emitter behind its IR constructor. One document so one module.
use "@LOWER@" { admit_src, admit_ambient }
use "@EMITTER@" { emit_rust_src }
use "@JSON@" { json_parse }

// The interchange IR arrives as JSON TEXT, because that is what crosses a crate
// boundary; `json_parse` builds the `cordis::Value` the emitter's `Any`
// parameter erases to, which is the same boxed representation stdlib/value.rvl's
// `@rs` accessors read back out.
pub fn emit_rust_from_ir(ir_text: Str) -> Str {
  return emit_rust_src(json_parse(ir_text))
}
"""


def _native_root_document(out_dir: Path) -> Path:
    """Write the emission root document into `out_dir` and return its path."""
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "revl_gate_native_root.rvl"
    path.write_text(
        NATIVE_ROOT_TEMPLATE
        .replace("@LOWER@", (ROOT / SELFHOST_ROOT).as_posix())
        .replace("@EMITTER@", (ROOT / SELFHOST_EMITTER).as_posix())
        .replace("@JSON@", (ROOT / SELFHOST_IR_CONSTRUCTOR).as_posix()),
        encoding="utf-8")
    return path


# The generated cargo test's fixture (roadmap item 146 condition 4).
#
# The crate's own `cargo test` is the no-Python half of the emitter evidence, and
# a byte comparison is evidence only if the expectation comes from the REFERENCE
# rather than from the thing under test. Both sides are therefore derived here,
# at generation time: the reference frontend stages the IR, the reference rust
# backend emits from it, and the emitted crate has to reproduce those bytes with
# no Python on the machine. Nothing is transcribed, so a frontend or backend
# change rewrites the test and reds the drift gate in the same wave.
#
# The probe is the one `tests/test_gate_crate_admit.py` already pins the
# byte-identity clause on, so the two halves of the evidence are held over the
# same program.
EMIT_PROBE_SOURCE = "fn id(x: Int) -> Int { return x }"


def emit_probe_document(rustemit) -> dict[str, str]:
    """`{source, ir, want}` for the generated emitter test."""
    from revl.compiler import compile_source  # noqa: PLC0415

    document = compile_source(EMIT_PROBE_SOURCE, "probe.rvl")
    return {
        "source": EMIT_PROBE_SOURCE,
        "ir": json.dumps(document),
        "want": rustemit.emit(document),
    }
...
def build() -> dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="revl_gate_crate_root_") as tmp:
        root = _native_root_document(Path(tmp))
        ir_document = compile_files([str(root)])
    selfhost_rs = rustemit.emit(ir_document)
    tables = frontier_tables()
    ir = ir_tables()
    admission = admission_tables()
    digest = source_digest()
    emit_probe = emit_probe_document(rustemit)
    return render(tables, digest, frontier_id(digest), language_version(),
                  selfhost_rs, ir, admission, emit_probe)
