...
/-- Byte index of `c` in `s`, or `s.length` when absent - a `Nat`-based
replacement for `String.find?` (whose result is a `String.Pos` here). -/
...
def groupCaps (pairs : List (String × String)) : List (String × List String) :=
  pairs.foldl (fun acc (k, c) =>
    match acc.find? (fun (n, _) => n == k) with
    | none => acc ++ [(k, [c])]
    | some _ => acc.map (fun (n, cs) => if n == k then (n, cs ++ [c]) else (n, cs)))
    []

def lookupCaps (m : List (String × List String)) (k : String) : List String :=
  match m.find? (·.1 == k) with
  | none => []
  | some (_, cs) => cs

/-- One closure step over the spawn edges: every parent absorbs its
children's caps. Monotone, so `n` passes for `n` components suffice. -/
def oneStep (edges : List (String × String))
             (closed : List (String × List String)) : List (String × List String) :=
  edges.foldl (fun acc (p, c) =>
    acc.map (fun (n, cs) =>
      if n == p then (n, union cs (lookupCaps acc c)) else (n, cs)))
    closed

def closeN (n : Nat) (edges : List (String × String))
             (closed : List (String × List String)) : List (String × List String) :=
  match n with
  | 0 => closed
  | n + 1 => closeN n edges (oneStep edges closed)

/-- The spawn-attenuation verdict per edge: the child's closed reach must
be covered by the spawner's held capabilities. -/
def attenOK (edges : List (String × String)) (_ownsOwn heldOwn closedOwn : List (String × List String))
             (parent child : String) : Bool :=
  let closed := closeN (edges.length + 1) (edges.eraseDups) closedOwn
  let childCaps := lookupCaps closed child
  let held := lookupCaps heldOwn parent
  childCaps.all fun c => held.any fun h => capCovers h c
...
def main (args : List String) : IO UInt32 := do
  ...
  | [inPath, outPath] =>
    ...
    for p in paths do
      ...
      let heldOwn := uk.foldl (fun acc r =>
        acc.map (fun (n, cs) => if n == r.comp then (n, union cs [r.cap]) else (n, cs)))
        owns
      ...
      for e in edges do
        let ok := attenOK edges owns heldOwn owns e.1 e.2
        let av := if ok then "ok" else "fail"
