...
def walk_u_call(node, out: list[tuple[str, str, str, str]], ctx: str,
                requires: dict, handles: dict, psvc: dict, services: dict) -> None:
    """Marker-rule call facts (U rows), extended to spawn-handle receivers:
    `(root, svc, meth, ctx)` for a call whose receiver is a require binding
    or a spawn handle."""
    if node is None or isinstance(node, (str, int, float, bool)):
        return
    if isinstance(node, (EmitStmt, EmitExpr)) and not isinstance(node, type):
        if dataclasses.is_dataclass(node):
            for f in dataclasses.fields(node):
                walk_u_call(getattr(node, f.name), out, "emit", requires,
                            handles, psvc, services)
        return
    if isinstance(node, ExprCall):
        rt = _route(node.callee)
        if rt:
            ...
            if res is not None:
                svc, meth = res
                if meth in services.get(svc, {}):
                    out.append((root, svc, meth, ctx))
        for a in node.args:
            walk_u_call(a, out, ctx, requires, handles, psvc, services)
        return
    if dataclasses.is_dataclass(node) and not isinstance(node, type):
        for f in dataclasses.fields(node):
            walk_u_call(getattr(node, f.name), out, ctx, requires, handles,
                        psvc, services)
        return
    if isinstance(node, (list, tuple)):
        for x in node:
            walk_u_call(x, out, ctx, requires, handles, psvc, services)
...
def export() -> tuple[list[str], dict[str, dict], dict[str, int]]:
    ...
    for path in corpus_files():
        ...
        for c in prog.components:
            ...
            # spawn facts: activation spawns are attenuation edges (S); every
            # spawn binding is a handle (H) for receiver resolution.
                ...
                if isinstance(stmt, ProvideStmt):
                    for pm in stmt.methods:
                        for inner in pm.body:
                            collect_spawns(inner, handles, [])
            for bind, child in sorted(dict(act_spawns).items()):
...
def checker_alignment(file_facts: dict, formal_files: dict,
    ...
    for rel in file_facts:
        ...
        if code == "accept":
            record("agree-accept" if formal_clean else "model-refuses-checker-accepts",
                   rel)
        elif code == "G4":
    ...
    for k in sorted(align):
        print(f"  {k:28} {align[k]}")
    for k in ("formal-strict", "missed-G4", "missed-G2",
              "model-refuses-checker-accepts"):
