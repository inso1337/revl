...
Two Java (cordis4j stub) processes over a Unix socket, both from examples/
outcome.rvl: a provider running DirSvc (serving `dir`, which returns the user
ADT `Found` and a `Result`), and a consumer running AskClient (proxying `dir`)
whose probes cross those tagged values back and rebuild the native variants.
Proves the `{"$kind","$value"}` codec both directions on the JVM.
...
# Provider-only fixture, inline. examples/outcome.rvl also has AskClient (an
# effect-less provide component wrapping `dir`), which a SEPARATE Java emitter
# bug miscompiles (new AskClientAsk() drops the injected `dir`, Object return).
# DirSvc (which has an effect) emits correctly, and the consumer here probes the
# proxied `dir` directly, so this isolates the ADT/Result codec.
FIXTURE = """
type Row = { id: Int, name: Str }
type Found = Hit(Row) | Missing
service Directory {
  fn lookup(id: Int) -> Found
  fn check(id: Int) -> Result[Row, Str]
}
component DirSvc provides dir: Directory {
  let store = effect Map.new() undo store.drop()
  provide dir {
    fn lookup(id) = Hit({ id: id, name: "ada" })
    fn check(id) = Ok({ id: id, name: "ada" })
  }
}
"""
...
def emit_components() -> None:
    ...
    (pkg / "Components.java").write_text(module.emit(compile_source(FIXTURE)), encoding="utf-8")
...
def main() -> int:
    ...
    cons_spec = _spec(PLACEMENT / "adt_cons.json", {
        "name": "cons", "module": "revl.Components", "components": [],
        "ifaces": {"dir": "revl.Components$Directory"},
        "proxies": {"dir": {"socket": sock}},
        "probe": ["dir.lookup(1)", "dir.check(1)"],
    })
    ...
    try:
        ...
        check("adt-hit", '"$kind":"Hit"' in text and '"id":1' in text and '"name":"ada"' in text,
              "dir.lookup(1) rebuilt Found.Hit(Row{id:1,name:ada}) across the seam")
        check("result-ok", '"$kind":"Ok"' in text and text.count('"name":"ada"') >= 2,
              "dir.check(1) rebuilt Result.Ok(Row) across the seam")
    finally:
