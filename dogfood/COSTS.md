...
| run | task | outcome | cost notes |
|---|---|---|---|
| harness-m2 | real HTTP provider + JSON protocol | landed 4d6750b | extern-dedent emitter bug (item 78) — the session's longest single stall; fixed upstream |
| harness-m3 | durable sessions + subagents | landed 805ce91 | spawn-emit frontend crash (item 82) — worked around by moving `emission` one level up |
| harness-m4 | fs tools + audit-diff approval gate | landed cf7242b | the gate refused fs_exfil by design — zero cost, the feature working |
| harness-m5 | web GUI shell | landed bd7d5c7 | no new findings; G6 ternary pattern fully automatic by now |
| harness-m6 | self-hosting admission + multi-session | landed ad4403a | host-Map keys compile-but-crash (item 84); escape-sequence test source (item 85) |
| harness-m7 | self-evolution | landed 1664b36 | extern/session module-boundary (docs note) |
| harness-m8 | self-hosting through the web shell | landed 9444e0e | no new findings |
| harness-m9 | real-model self-evolution | landed d0943eb | evolver once() had to decode the JSON wire reply — a harness design fix, not a revl gap |
| harness-m10 | multi-session web UI | landed f1c9b62 | run_in(session,prompt); default-vs-named session coherence in tests |
